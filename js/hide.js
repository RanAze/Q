//时刻关注现在执行的是哪个定时器
  window.onload=function(){
    ...
    var oDiv88=document.getElementById('leftNav');
    var oDiv2=document.getElementById('div2');
    var oDiv22=document.getElementById('logo');
    var oDiv33=document.getElementById('logo1');
  ...
  console.log(scrollTop,initTop)
      ...
      {
        oDiv88.style.display='block';
        oDiv22.style.display='none';
        oDiv33.style.display='block';
        oDiv2.style.background='#fff';
    ...
    var oDiv99=document.getElementById('phone');
    ...
    oA.onmouseover=function()
    {
      ...
      timer=setTimeout(function(){
        oDiv99.style.display='block';
      },100);//当鼠标在按钮上停留300毫秒后div显示
    };
    oA.onmouseleave=function()
    {
      clearTimeout(timer);//若鼠标只是滑过按钮则关闭上一个定时器300毫秒后让div不显示
      timer=setTimeout(function(){
        oDiv99.style.display='none';
      },300);//若div正常显示则让它1s后消失
    };
    oDiv99.onmouseenter=function(){
      clearTimeout(timer);//若鼠标悬停在div上则关闭上一个定时器让div一直显示
    };
    oDiv99.onmouseleave=function(){
      timer=setTimeout(function(){
        oDiv99.style.display='none'
      },300);//若鼠标离开div则让div在1s后消失
    };
